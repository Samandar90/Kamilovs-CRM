...
const parseAppointmentPatientName = (message) => {
...
const parsePatientName = (message) => {
    const m = message.match(/(?:добавь|создай|зарегистрируй)\s+пациент[а-я]*\s+([А-ЯA-ZЁ][^,.\n]{1,120})/i) ??
...
const parseDoctorName = (message) => {
    const mDoc = message.match(/\bк\s+(?:доктору|врачу)\s+([А-ЯA-ZЁ][А-ЯA-ZЁа-яa-z\-\s]{0,60})/i) ??
...
const parseServiceName = (message) => {
...
const parsePaymentMethod = (message) => {
...
const parseInvoiceRef = (message) => {
    const byNum = message.match(/(?:счет|инвойс|invoice)\s*#?\s*([a-zа-я0-9\-]+)/i);
    return byNum?.[1]?.trim();
};
function parseAction(message, intent) {

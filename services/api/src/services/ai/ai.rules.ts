...
const sum = (value: number): string => `${Math.round(value).toLocaleString("ru-RU")} сум`;

export function handleRuleBased(intent: Intent, message: string, context: AIContext): string | null {
  ...
  if (intent === "CRM_HELP") {
    if (text.includes("запис")) return "Создание записи: раздел «Записи» → «Новая запись» → выбрать пациента, врача и услугу.";
    if (text.includes("счет")) return "Создание счета: раздел «Биллинг / Счета» → «Создать счет» → выбрать пациента и услуги.";
    if (text.includes("касс")) return "Касса: раздел «Биллинг / Касса». Сначала откройте смену, затем проводите оплаты.";

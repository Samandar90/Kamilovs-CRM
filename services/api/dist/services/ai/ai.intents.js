...
function detectIntent(message) {
    ...
    if (!text)
        return "CHAT";
    if (hasAny(text, CLOSE_SHIFT_HINTS))
        return "CLOSE_SHIFT";
    if (hasAny(text, CREATE_PAYMENT_HINTS))
        return "CREATE_PAYMENT";
    if (hasAny(text, CREATE_INVOICE_HINTS))
        return "CREATE_INVOICE";
    if (hasAny(text, CREATE_APPOINTMENT_HINTS))
        return "CREATE_APPOINTMENT";
    if (hasAny(text, CREATE_PATIENT_HINTS))
        return "CREATE_PATIENT";
    if (text.includes("покажи выручку"))
        return "GET_REVENUE";
    if (text.includes("сколько долгов") || text.includes("сколько долга"))
        return "GET_DEBTS";
    if (hasAny(text, GET_DOCTORS_HINTS) || text.includes("врачи"))
        return "GET_DOCTORS";
    if (hasAny(text, SYSTEM_ISSUE_HINTS))
        return "SYSTEM_ISSUE";
    if (hasAny(text, MEDICAL_HINTS))
        return "MEDICAL";
    if (hasAny(text, CRM_ANALYTICS_HINTS))
        return "CRM_ANALYTICS";
    if (hasAny(text, CRM_QUERY_HINTS))
        return "CRM_QUERY";
    if (hasAny(text, CRM_HELP_HINTS))
        return "CRM_HELP";
    if (/^как\s+создат[ьа]/.test(text) || /^где\s+/.test(text))
        return "CRM_HELP";
    if (/выруч|счет|оплат|касс|врач|пациент|услуг|запис/.test(text))
        return "CRM_QUERY";
    return "CHAT";
}

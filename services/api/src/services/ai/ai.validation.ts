...
export type AIValidationDependencies = {
  patientsService: {
    list: (auth: AuthTokenPayload, options?: { search?: string }) => Promise<Array<{ id: number; fullName: string }>>;
  };
  doctorsService: {
    list: (auth: AuthTokenPayload) => Promise<Array<{ id: number; name: string }>>;
  };
  servicesService: {
    list: (auth: AuthTokenPayload, filters?: { doctorId?: number }) => Promise<Array<{ id: number; name: string }>>;
  };
  appointmentsService: {
    list: (
      auth: AuthTokenPayload,
      filters?: { patientId?: number; doctorId?: number; startFrom?: string; startTo?: string }
    ) => Promise<Array<{ id: number; patientId: number; doctorId: number; serviceId: number; startAt: string; status: string }>>;
  };
  invoicesService: {
    list: (auth: AuthTokenPayload) => Promise<Array<{ id: number; number: string; status: string; total: number; paidAmount: number }>>;
  };
  cashRegisterService: {
    getActiveShift: (auth: AuthTokenPayload) => Promise<{ id: number } | null>;
  };
};

export type ValidatedAction =
  | { type: "CREATE_PATIENT"; payload: { fullName: string } }
  | {
      type: "CREATE_APPOINTMENT";
      payload: { patientId: number; doctorId: number; serviceId: number; startAt: string };
    }
  | { type: "CREATE_INVOICE"; payload: { patientId: number; appointmentId: number; serviceId: number } }
  | { type: "CREATE_PAYMENT"; payload: { invoiceId: number; amount: number; method: "cash" | "card" } }
  | { type: "CLOSE_SHIFT"; payload: { shiftId: number } };
...
const toStartAt = (date: string, time: string): string => {
  const t = time.trim();
  const normalizedTime = t.length === 5 ? `${t}:00` : t;
  return `${date} ${normalizedTime}`;
};

const pickSingleByName = <T extends { id: number }>(
  items: T[],
...
export class AIValidationService {
  constructor(private readonly deps: AIValidationDependencies) {}

  async validateAction(action: AIAction, auth: AuthTokenPayload): Promise<ValidationResult> {
    ...
    if (action.type === "CREATE_APPOINTMENT") {
      if (!action.payload.doctorName) return { ok: false, message: "Какой врач?" };
      if (!action.payload.date) return { ok: false, message: "Уточните дату" };
      const time = action.payload.time?.trim() || "10:00";

      let patient: { id: number; fullName: string } | null = null;
      ...
      if (nameQuery) {
        ...
        if (!patient) return { ok: false, message: "Не найден пациент" };
      } else {
        ...
        if (all.length === 1) {
          ...
          return {
            ok: false,
            message:
              "Укажите пациента в запросе (например: «для Иванова И.И.» или «пациент Сидоров»).",
          };
        }
      }
      ...
      if (!doctor) return { ok: false, message: "Не найден врач" };

      const doctorServices = await this.deps.servicesService.list(auth, { doctorId: doctor.id });
      if (doctorServices.length === 0) return { ok: false, message: "У врача нет доступных услуг для записи" };
      const service = action.payload.serviceName
        ? pickSingleByName(doctorServices, action.payload.serviceName, (x) => x.name)
        : doctorServices[0];
      if (!service) return { ok: false, message: "Не найдена услуга для записи" };
      ...
      if (hasConflict) return { ok: false, message: "У врача уже есть запись на это время" };
    ...
    if (action.type === "CREATE_INVOICE") {
      if (!action.payload.patientName) return { ok: false, message: "Не найден пациент" };
      ...
      if (!candidate) return { ok: false, message: "Нет подходящей записи для создания счета" };
      ...
      if (action.payload.serviceName) {
        ...
        if (!svc) return { ok: false, message: "Не найдена услуга" };
        serviceId = svc.id;
      }
    ...
    if (action.type === "CREATE_PAYMENT") {
      if (!action.payload.amount || action.payload.amount <= 0) return { ok: false, message: "Уточните сумму оплаты" };
      const rawMethod = action.payload.paymentMethod ?? "cash";
      const method: "cash" | "card" = rawMethod === "cash" ? "cash" : "card";

      const activeShift = await this.deps.cashRegisterService.getActiveShift(auth);
      if (!activeShift) return { ok: false, message: "Сначала откройте кассовую смену" };

      if (action.payload.amount >= 1_000_000 && !action.payload.confirmed) {
      ...
      if (payable.length === 0) return { ok: false, message: "Нет счетов для оплаты" };
      ...
      if (action.payload.invoiceRef) {
        ...
        if (!byNumber) return { ok: false, message: "Счет не найден" };
        target = byNumber;
      }
      return {
        ok: true,
    ...
    if (action.type === "CLOSE_SHIFT") {
      const activeShift = await this.deps.cashRegisterService.getActiveShift(auth);
      if (!activeShift) return { ok: false, message: "Активная смена не найдена" };
      if (!action.payload.confirmed) return { ok: false, message: "Подтвердите закрытие смены словом «подтверждаю»" };
